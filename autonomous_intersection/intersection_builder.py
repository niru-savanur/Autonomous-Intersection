...
class IntersectionBuilder:
    ...
    def generate(width: float, height: float, road_width: float, side_width: float, line_width: float, model):
        ...
        yield VisualCell((x_pos[0] + side_width + road_width / 2 - line_width / 2, 0), (line_width, y_pos[0] + side_width), model, color="white")
        yield VisualCell((x_pos[0] + side_width + road_width / 2 - line_width / 2, y_pos[1]), (line_width, width - y_pos[1]), model, color="white")

        yield VisualCell((0, y_pos[0] + side_width + road_width / 2 - line_width / 2), (x_pos[0] + side_width, line_width), model, color="white")
        yield VisualCell((x_pos[1], y_pos[0] + side_width + road_width / 2 - line_width / 2), (width - x_pos[1], line_width), model, color="white")
